...
def run_nii2dcm(input_nii_path, output_dcm_path, dicom_type=None):
    ...
    # take the input data and scale them to be in a range of 0 to 2^12 - 1
    # get the min and max values from the data
    max_val = nii_img.max()
    min_val = nii_img.min()

    # try simple scaling first
    scale_factor = 4095 / (max_val - min_val)
    nii_img = (nii_img - min_val) * scale_factor
    ...
    for instance_index in range(0, nii2dcm_parameters["NumberOfInstances"]):
        ...
        nii2dcm.dcm_writer.transfer_nii_hdr_instance_tags(
